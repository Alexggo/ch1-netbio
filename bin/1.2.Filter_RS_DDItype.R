...
raw <- read_csv2("data/1.processed/Broc2018_allddi_set.csv")
x <-  raw |> 
  select(1:3,19:62)
...
raw |> filter(drug_pair %in% only_sen_DDI) |> 
  write_csv2("data/1.processed/Broc2018_onlysen_set.csv")

raw |> filter(drug_pair %in% set2) |> 
  write_csv2("data/1.processed/Broc2018_sen2in1_set.csv")
