...
list_Kedge <- list()
for (i in 1:length(filenames)){
  list_Kedge[[i]] <- df_tot %>% filter(network==net[i]) %>% 
...
b <- df_tot %>%
  filter(network==net[1]|network==net[10]) %>% 
  ggline(x = "mean.path.length", y = "sigma.rate", add = c("mean_se"),
                  group = "network", col="network",linetype = 9)+
  xlab("Path length")+
  ylab("Sigma rate")+
  theme_minimal()+
  theme(legend.position = "bottom")
...
# 20 x 10. SupFig
wrap_plots(sup_a,sup_b,sup_c,sup_e,sup_f,sup_g,sup_d,ncol=3)+ 
  plot_annotation(tag_levels = 'A',tag_suffix = '.')&
   theme(plot.tag.position = c(0, 1),
