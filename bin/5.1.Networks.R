...
"Similargenomiccontext", "Co-functional(GO-BP)",
"Co-citation", "Co-expression",
"Co-occurenceofprot.domains", "Co-functional(EcoCyc/GO-BP)",
"High-throughputPPI", "Small/medium-scalePPI",
"Similarphylogeneticprofiles")
...
#
##Remove nodes with degreegreater than 90%.
#list_graphs <- list_net |>
#map(graph_from_data_frame, directed = FALSE, vertices = NULL)
#trimmed_graph <- list()
#for(i in seq_along(l_deg_list)) {
...
target_list <- list()
drug_list <- list()
for (i in seq_along(shortest_paths)) {
...
print(paste("Numberoftargets:", Tar))
print(targets)
D <- drugs |>length()
print(paste("Numberofdrugs:", D))
drugs |>print()
#For small matrix of targets from Brochado, usedist_mat[r, c]
distnew2 <- dist_mat |>
as.data.frame()
#Convert matrixtotable
...
pivot_longer(names_to = "KEGG2",
values_to = "path.length", 2:(dim(distnew2)[2])) |>
filter(!is.na(path.length))
}


#k.edge.connectivitybetweenknownnodesthatareing1
list_Kedge <- list()
for (i in seq_along(list_graphs)) {
#For small set use:
...
for (j in seq_along(selection)) {
  for (k in seq_along(selection)) {
    if (j == k) {t[j, k] <- NA}
      else{
t[j, k] <- edge_connectivity(list_graphs[[i]],
source = selection[j], target = selection[k])
...
#Merge both path length and Kedge connectivity.
list_path <- list_path |>
...
distnew3 <- distnew2 |>
pivot_longer(names_to = "KEGG2",
values_to = "adjacency", 2:(dim(distnew2)[2])) |>
