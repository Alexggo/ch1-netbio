...
# Results per strain:
str_fig <- 1
fig1_str <- wrap_plots(plot_net_int(1,1,str_fig,c(T,F)),plot_net_int(3,1,str_fig,c(T,F)),
...
var_fig <- 1
fig1_var <- wrap_plots(plot_net_int(1,var_fig,1,c(T,F)),plot_net_int(3,var_fig,1,c(T,F)),
...
gr <- fig.4a.df$type |> unique()
g1 <- fig.4a.df |> filter(type==gr[1]) |> select(sigma.rate) |> pull()
g2 <- fig.4a.df |> filter(type==gr[2]) |> select(sigma.rate) |> pull()
g3 <- fig.4a.df |> filter(type==gr[3]) |> select(sigma.rate) |> pull()
g4 <- fig.4a.df |> filter(type==gr[4]) |> select(sigma.rate) |> pull()
g5 <- fig.4a.df |> filter(type==gr[5]) |> select(sigma.rate) |> pull()
g6 <- fig.4a.df |> filter(type==gr[6]) |> select(sigma.rate) |> pull()

t_1_2 <- t.test(g1,g2) |> report() #p < 0.002
t_1_3 <- t.test(g1,g3) |> report() #p < .001
t_1_4 <- t.test(g1,g4) |> report() #p < .001
t_1_5 <- t.test(g1,g5) |> report() #p < .001
t_1_6 <- t.test(g1,g6) |> report() # Not enough observations

t_2_3 <- t.test(g2,g3) |> report() #p < .001
t_2_4 <- t.test(g2,g4) |> report() #p < 0.526. "Additivity-Antagonism-Synergy" vs "Additivity-Antagonism"
t_2_5 <- t.test(g2,g5) |> report() #p < .001
t_2_6 <- t.test(g2,g6) |> report() # Not enough observations

t_3_4 <- t.test(g3,g4) |> report() #p < .001
t_3_5 <- t.test(g3,g5) |> report() #p < .001
t_3_6 <- t.test(g3,g6) |> report() # Not enough observations

t_4_5 <- t.test(g4,g5) |> report() #p < .001
t_4_6 <- t.test(g4,g6) |> report() # Not enough observations

t_5_6 <- t.test(g5,g6) |> report() # Not enough observations
...
fig.4b.df <- df |> 
  select(sum_g,sigma.rate) |> 
  filter(!is.na(sum_g)) |> 
  mutate(sum_g1=cut(sum_g,breaks=c(-7,-2,+2,+7))) 
...
ad <- fig.4b.df |> filter(sum_g1=="(-2,2]") |> select(sigma.rate) |> pull()
an <- fig.4b.df |> filter(sum_g1=="(-7,-2]") |> select(sigma.rate) |> pull()
...
t1 <- t.test(n1.s,n1.ad) #p < .001
t2 <- t.test(n1.s,n1.an) #p < .001
t3 <- t.test(n1.ad,n1.an) #p = 0.001775
...
t1 <- t.test(n2.s,n2.ad) #p < .001
t2 <- t.test(n2.s,n2.an) #p < .001
t3 <- t.test(n2.ad,n2.an) #p = 0.852
...
fig.4d.df1 <- fig.4d.df |> 
  ...
  mutate(path.length.corr1=ifelse(path.length.corr==">6",6,path.length.corr)) |> 
  select(network,sigma.rate,path.length.corr1)
...
write.csv(df1,"results/allddi/example_DDI.csv")
```
