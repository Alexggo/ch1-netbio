...
p_load(igraph,tidyverse,BioNet,ape,geomorph,matrixStats,
       ComplexHeatmap,ggpubr,plotrix,tidymodels,patchwork,here,EnvStats)
options(warn = -1)
set_name <- "allddi"
# VISUALIZATION/STATS
...
df_DDI_tot <- read_csv(paste0("results/","df_DDI_metrics",".csv")) |> 
  mutate(int_sign_ebw=factor(int_sign_ebw,levels=c("Synergy","Additivity","Antagonism","Sample_nodes"))) |> 
  mutate(int_sign_ecr=factor(int_sign_ecr,levels=c("Synergy","Additivity","Antagonism","Sample_nodes"))) |> 
  mutate(int_sign_stm=factor(int_sign_stm,levels=c("Synergy","Additivity","Antagonism","Sample_nodes"))) |> 
  mutate(int_sign_seo=factor(int_sign_seo,levels=c("Synergy","Additivity","Antagonism","Sample_nodes"))) |> 
  mutate(int_sign_pae=factor(int_sign_pae,levels=c("Synergy","Additivity","Antagonism","Sample_nodes"))) |> 
  mutate(int_sign_pau=factor(int_sign_pau,levels=c("Synergy","Additivity","Antagonism","Sample_nodes"))) |> 
...
my_comparisons <- list( c("Synergy", "Additivity"), c("Additivity", "Antagonism"), c("Synergy", "Antagonism"),c("Synergy", "Sample_nodes"), c("Additivity", "Sample_nodes"), c("Antagonism", "Sample_nodes"))
...
# Fixed below this
df_target1 <- left_join(df_target_tot,df_DDI_tot,by="DRUG_ID") |> 
...
df_comb <- left_join(df,df_target1,by="DRUG_ID")
# mean path ~ interaction (Syn,Ant,Add,Add-Ant,Ant-Syn,Add-Syn,Add-Ant-Syn)

# Mean.rate. 5A
a <- df_comb |>
  select(type,sigma.rate_all)  |> 
  group_by(type) |> 
  summarise(Mean.rate=mean(sigma.rate_all),
            sem.rate=std.error(sigma.rate_all),
            count=n()) |> 
  ungroup() |> 
...
c <- df_target1 |>
  ggline(x = "int_sign_ebw.x", y = "sigma.rate_all.y", add = "mean_se",
...
d <- df_target1 |>
  ...
  ggline(x = "path.length.corr", y = "sigma.rate_all.x", add = c("mean_se"),
...
df_target1 |> ggline(x = "sum_g.y", y = "sigma.rate_all.y", add = "mean_se",
...
sup_d <- df_DDI_tot |>
  ...
  select(network,sigma.rate_all,clusters,DRUG_ID,connection_onoff,type,mean.path.length)  |> 
  distinct() |> 
...
sup_e <- df_DDI_tot |>
  filter(network==net[1]|network==net[3]) |> 
  select(network,sigma.rate_all,clusters,DRUG_ID,connection_onoff,type,mean.path.length,mean.k.edge)  |> 
  filter(!is.infinite(mean.path.length)) |> 
  distinct() |> 
  group_by(type,network) |> 
...
sup_i <- df_DDI_tot |>
  ...
  ggline(x = "mean.k.group", y = "sigma.rate_all", add = "mean_se",
         group = "network", col="network")+
...
sup_j <- df_DDI_tot |>
  ...
  ggline(x = "mean.deg.group", y = "sigma.rate_all", add = "mean_se",
         group = "network", col="network")+
...
df_DDI_tot |> 
  ...
  filter(Drug2 %in% c("A22","Novobiocin","Ciprofloxacin",
                      "Meropenem","Sulfamonomethoxine","Trimethoprim")) |> 
  select(network,DRUG_ID,clusters,sigma.rate_all,
