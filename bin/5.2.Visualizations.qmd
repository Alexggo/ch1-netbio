...
df_DDI_tot <- read_csv2(paste0("results/","df_DDI_metrics",".csv")) |> 
  mutate(int_sign_ebw=factor(int_sign_ebw,levels=c("Synergy","Additivity","Antagonism","Non-target nodes"))) |> 
  mutate(int_sign_ecr=factor(int_sign_ecr,levels=c("Synergy","Additivity","Antagonism","Non-target nodes"))) |> 
  mutate(int_sign_stm=factor(int_sign_stm,levels=c("Synergy","Additivity","Antagonism","Non-target nodes"))) |> 
  mutate(int_sign_seo=factor(int_sign_seo,levels=c("Synergy","Additivity","Antagonism","Non-target nodes"))) |> 
  mutate(int_sign_pae=factor(int_sign_pae,levels=c("Synergy","Additivity","Antagonism","Non-target nodes"))) |> 
  mutate(int_sign_pau=factor(int_sign_pau,levels=c("Synergy","Additivity","Antagonism","Non-target nodes"))) |> 
...
my_comparisons <- list( c("Synergy", "Additivity"), c("Additivity", "Antagonism"), c("Synergy", "Antagonism"),c("Synergy", "Non-target nodes"), c("Additivity", "Non-target nodes"), c("Antagonism", "Non-target nodes"))
...
plot_net_num <- function(net_sel,var_sel,str_name){
  ...
  min_k <- df_DDI_tot |> 
    ...
    br_df1 <- df1 |> 
      ...
      mutate(Additivity=ifelse(is.na(`Non-target nodes`),0,`Non-target nodes`)) |> 
      rowwise() |> 
...
sup_d <- df_DDI_tot |>
  ...
  mutate(type=ifelse(is.na(type),"Non-target nodes",type)) |>
