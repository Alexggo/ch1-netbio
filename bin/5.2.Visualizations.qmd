...
options(warn = -1)
set_name <- "allddi" #allddi, set2in1
set_col <- "all" #all, set
# VISUALIZATION/STATS
...
plot_net_num <- function(net_sel,var_sel,str_name){
  ...
  min_k <- df_DDI_tot |> 
    ...
    br_df1 <- df1 |> 
      ...
      mutate(Additivity=ifelse(is.na(Sample_nodes),0,Sample_nodes)) |> 
      rowwise() |> 
...
sup_d <- df_DDI_tot |>
  ...
  mutate(type=ifelse(is.na(type),"Sample_nodes",type)) |>
...
# 20 x 20. SupFig
SupFig <- wrap_plots(sup_a,sup_b,sup_c,sup_d,sup_e,sup_f,sup_g,
...
pdf(paste0("results/",set_name,"/figures_",set_col,".pdf"),
    width=10, height=10)
fig4
SupFig
dev.off()
