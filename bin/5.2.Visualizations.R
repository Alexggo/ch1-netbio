...
net_names <- c("Co-functional(EcoCyc)",
               "Co-functional (EcoCyc/GO-BP)",
               "Small/medium-scale PPI",
               "STRINGDB-full-ebw", "STRINGDB-physical-ebw",
...
strains <- c("int_sign_ebw","int_sign_ecr","int_sign_pae","int_sign_pau","int_sign_seo","int_sign_stm")
...
pdf("results/allddi/fig3_allnetworks.pdf",width = 15,height = 15)
fig3_allnetworks
dev.off()
...
fig3_setnetworks <- list()
str_fig_vec <- c(1,1,1,1,1,2,2,3,3,6,6)
for (net_now in seq_along(net_names)){
  str_fig <- str_fig_vec[net_now]
  fig3_setnetworks[[net_now]] <-   wrap_plots(plot_net_int(net_now,1,1,T),plot_net_int(net_now,2,str_fig,T),
                                              plot_net_int(net_now,3,str_fig,T),plot_net_int(net_now,4,str_fig,T),
                                              plot_net_int(net_now,5,str_fig,T),plot_net_int(net_now,6,str_fig,T))+ 
    plot_annotation(tag_levels = 'A',tag_suffix = '.')&
    theme(plot.tag.position = c(0, 1),
          plot.tag = element_text(size = 15, hjust = 0, vjust = 0))+
    theme(axis.title = element_text(size = 12),  # Set axis title size
...
pdf("results/allddi/fig3_setnetworks.pdf",width = 15,height = 15)
fig3_setnetworks
dev.off()
...
# Rates ~ Interaction Type (per strain) (PPI). 4C
fig4c_networks <- list()
str_fig_vec <- c(1,1,1,1,1,2,2,3,3,6,6)
st_name <- sub(".+_(.+)", "\\1", strains)
for (net_now in seq_along(net_names)){
  str_fig <- str_fig_vec[net_now]
  st <- paste0(strains[str_fig],".DDI")
  print(net_names[net_now])
  print(st)
 
  fig.4c.df <- df_target_DDI |>
    select(network,all_of(st),starts_with("sigma.rate")) |> 
    select(network,all_of(st),ends_with(paste0(set_col,".DDI"))) |> 
    rename(sigma.rate=starts_with("sigma.rate")) |> 
    filter(!is.na(!!sym(st))) |> 
    mutate(int_sign=!!sym(st))
  
  fig4c_networks[[net_now]] <- fig.4c.df |>
    ggline(x = "int_sign", y = "sigma.rate", add = "mean_se",
           group = "network", col="network")+
    xlab(paste0("Type of interaction (",st_name[str_fig_vec[net_now]],")"))+
    ylab(expression("Sigma rate ("~Bliss^2/MYA~")"))+
    theme_minimal()+
    theme(legend.position = "bottom")+
    ggtitle(net_names[net_now])
}

pdf("results/allddi/fig4c_networks.pdf",width = 15,height = 15)
fig4c_networks
dev.off()
...
# Network 1:
fig.4d.df1.n1 <- fig.4d.df1 |> 
  filter(network=="Co-functional (EcoCyc/GO-BP)") |> 
  group_by(path.length.corr1) |> 
  summarise(Mean.sigma=mean(sigma.rate,na.rm=T),
